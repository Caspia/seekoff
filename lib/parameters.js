...
module.exports.preferenceDefaults = {
  ...
  'git eslint jasmine',
  tagsToExclude: 'exploit sql-injection penetration-testing xss sniff attack crack trojan code-injection malware keylog keylogger virus',
  port: 8080,
  indexTimeout: 30000,
  xmlFilePath: '',
};
...
let parametersOld = {};
try {
  const prefsPath = path.join(require('os').homedir(), '.seekoff', 'prefs.json');
...
parms.elasticHost = process.env.SEEKOFF_ELASTIC_HOST || parms.elasticHost;
parms.indexPrefix = process.env.SEEKOFF_INDEX_PREFIX || parms.indexPrefix;
parms.tagsToInclude = process.env.SEEKOFF_TAGS_TO_INCLUDE || parms.tagsToInclude;
parms.tagsToExclude = process.env.SEEKOFF_TAGS_TO_EXCLUDE || parms.tagsToExclude;
parms.port = process.env.SEEKOFF_PORT || parms.port;
parms.indexTimeout = process.env.SEEKOFF_INDEX_TIMEOUT || parms.indexTimeout;
parms.xmlFilePath = process.env.SEEKOFF_XML_FILE_PATH || parms.xmlFilePath;
