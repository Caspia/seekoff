 ...
 */
async function indexFromPostIds(questionsPath, client, type, indexPrefix, onProgress) {
  ...
  const shouldIndexByType = {
    ...
    secomment: async lineObject => elasticClient.multiExists.promiseExists(client, postIndex, 'sepost', lineObject.PostId),
    seuser: async lineObject => Number(lineObject.Id) >= 0,
    sepostlink: async lineObject => {
      const [hasPostId, hasRelatedId] = await Promise.all([
        elasticClient.multiExists.promiseExists(client, postIndex, 'sepost', lineObject.PostId),
        elasticClient.multiExists.promiseExists(client, postIndex, 'sepost', lineObject.RelatedPostId),
      ]);
  ...
  return readFile(
    ...
    () => {
      elasticClient.multiExists.flushRequests(client, postIndex, 'sepost')
        .catch((onrejected) => {
          console.log('Error flushing multiExists: ' + onrejected);
        });
    },
  );
}
