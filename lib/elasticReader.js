    ...
    async (lineObject) => {
      if (await shouldIndex(lineObject)) {
        ...
        if (type == 'sepost' && voteTotals) {
          lineObject.VoteCount = voteTotals.get(Number(lineObject.Id)) || 0;
        }
        return elasticClient.putDocument(client, indexPrefix + type, type, lineObject);
      }
    },
...
async function getAllPostIds(questionsPath, onProgress) {
  ...
  await readFile(
    postLinksPath,
    async (lineObject) => {
      if (questionIds.has(Number(lineObject.PostId))) {
        documentCount++;
        postIds.add(Number(lineObject.RelatedPostId));
      }
      if (questionIds.has(Number(lineObject.RelatedPostId))) {
        documentCount++;
