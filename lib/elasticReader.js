...
// Setup appropriate function to determine if an sepost matches a tags string
function makeDoesMatch(tagsString, isAccept) {
  ...
  const includes = tagsString.toLowerCase().split(' ');
  const stemmedIncludes = includes.map(include => stemmer(include));

  return (lineObject) => {
    try {
      //console.log('lineObject is: ' + JSON.stringify(lineObject));
      ...
      const aHit = stemmedIncludes.some(word => {
        //console.log('testing ' + exclude + ' in title:' + titleWords + ' tags: ' + tagWords);
        return (titleWords && titleWords.includes(word)) || (tagWords && tagWords.includes(word));
      });
      // only log rejects
      if (aHit && !isAccept) {
