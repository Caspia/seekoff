...
// Setup appropriate function to determine if an sepost should be rejected by tags.
function makeShouldReject() {
  // Pre-calculate regular expressions to detect each tag.
  const tags = parameters.tagsToExclude.split(' ');
  ...
  return (lineObject) => {
    try {
      ...
      if (!title) {
        const textBody = html2plaintext(lineObject.Body);
        title = (textBody.length > 60 ? textBody.substr(0, 60) + '...' : textBody);
      }
      const rejectMe = tags.some(tag => {
        const matchesTag =
          tagsRE[tag].test(title) ||
          (lineObject.Tags && tagsRE[tag].test(lineObject.Tags));
        return matchesTag;
      });
      if (rejectMe) {
        console.log('rejected post with tags ' + (lineObject.Tags || '') + ' title ' + title);
      }
      return rejectMe;
    } catch (err) {
      console.log('Error in reject by tag: ' + err);
      return true;
    }
  };
}
  ...
  if (answers.hits.hits.length) {
    ...
    if (question.found && question._source.PostTypeId == POSTTYPEID_QUESTION) {
      ...
      answers.hits.hits.forEach(hit => {
        ...
        indexPromises.push(elasticClient.promisePutDocument(client, indexPrefix + 'sepost', 'sepost', answerDocument));
      });
...
async function indexFromPostIds(postIdsPath, client, type, indexPrefix, onProgress) {
  ...
  const shouldReject = makeShouldReject();
  ...
  const shouldIndexByType = {
    sepost: async lineObject => postIds.has(lineObject.Id) && !shouldReject(lineObject),
...
async function getQuestionIdsByTags(filePath, tags, onProgress) {
  ...
  async function processPost(lineObject) {
    ...
    if (tags.some((tag) => {
      ...
      // Tag matches for this question, but should we reject?
      if (!shouldReject(lineObject)) {
