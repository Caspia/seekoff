...
const elasticClient = module.exports = {
  /**
  ...
  */
   ...
   */
  createIndex: function createIndex(client, index, type) {
   ...
   */
  deleteIndex: function deleteIndex(client, name) {
   ...
   */
  getIndicies: async function getIndicies(client) {
   ...
   */
  putDocument: function putDocument(client, index, type, body) {
    return limit(() => elasticClient.promisePutDocument(client, index, type, body));
  },

  promisePutDocument: function promisePutDocument(client, index, type, body) {
  ...
  promiseRefreshIndex: function promiseRefreshIndex(client, index) {
    return new Promise((resolve, reject) => {
   ...
   */
  getDocuments: async function getDocuments(client, index, type, ids) {
    return client.mget({
      index,
      type,
      body: { ids },
   ...
   */
  getDocument: async function getDocument(client, index, type, id) {
    return client.get({index, type, id});
  },

  getAllDocuments: async function getAllDocuments(client, index) {
  ...
  answers: async function answers(client, index, parentId, params) {
    const defaults = {
      index,
      body: {
        sort: [
  ...
  comments: async function comments(client, index, postId, params) {
    const defaults = {
      index,
      body: {
        query: {
  ...
  links: async function links(client, index, postId, params) {
    return elasticClient.comments(client, index, postId, params);
  },

  related: async function related(client, index, postId, params) {
    const defaults = {
      index,
      body: {
        query: {
  ...
  makeQuery: function makeQuery(searchString) {
   ...
   * @returns {Promise.<object>} client.search output 
   */
  search: async function search(client, index, searchString, params) {
    const defaults = {
      index,
      body: {
        query: elasticClient.makeQuery(searchString),
        highlight: {
          fields: {
            Tags: {},
            Title: {},
  ...
  explain: async function explain(client, index, searchString, type, id, params) {
    const defaults = {
      index,
      id,
      type,
...
/** @type {ElasticClient} */
module.exports.client = module.exports.makeClient({});
