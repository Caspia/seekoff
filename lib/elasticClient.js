...
const {MAX_READ_LINES, INDEX_TIMEOUT} = require('./constants.js');
...
function putDocument(client, index, type, body) {
  return limit(() => new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => reject(new Error('putDocument timeout')), INDEX_TIMEOUT);
...
const multiExists = {
  ...
  /**
   * Does a particular _id exist in an index?
   * @param {ElasticClient} client
   * @param {ElasticIndex} index
   * @param {ElasticType} type
   * @param {string} id - Does this _id exist in the index?
   * @returns {boolean} - true if id exists
   */
  promiseExists: async function (client, index, type, id) {
    return new Promise((resolve, reject) => {
      ...
      let indexEntry = clientEntry[index + type];
      if (!indexEntry) {
        // Start a timeout timer to make sure that this flushes eventually.
        const timer = setTimeout(() => {
          console.log('timeout in multiExists');
          this.flushRequests(client, index, type);
        }, this.TIMEOUT);
        indexEntry = {index, type, idRequests: [], timer};
        clientEntry[index + type] = indexEntry;
      }
      ...
      if (indexEntry.idRequests.length >= this.BATCH_COUNT) {
        this._issueRequest(client, index, type, indexEntry);
      }
    });
  },
  /**
   ...
   */
  flushRequests: async function (client, index, type) {
    try {
      const clientEntry = this.pendingRequests.get(client);
      const indexEntry = clientEntry ? clientEntry[index + type] : null;
      if (!indexEntry) {
        // nothing to do`
        return;
      }
      return this._issueRequest(client, index, type, indexEntry);
    } catch (err) {
   ...
   */
  _issueRequest: async function (client, index, type, indexEntry) {
    if (this.pendingRequests.get(client)[index + type]) {
      delete this.pendingRequests.get(client)[index + type];
    } else {
    ...
    */
    ...
    // Convert response into a simple Set
    const foundIds = new Set();
    response.docs.forEach((value) => {
      if (value.found) {
        foundIds.add(value._id);
      }
    });
